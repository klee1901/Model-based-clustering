n <- length(results)
vis <- data.frame(rep(NA,2*n),rep(NA,2*n),c(rep('gibbs',n),rep('collapsedGibbs',n)),c(1:n,1:n))
colnames(vis) <- c('dist','ESS','type','sample')
...
# summarise
ggplot(vis,aes(type,dist))+geom_boxplot(aes(color=type))+labs(x='algorithm',y='VI distance',title='Comparison of VI distances for both types of samplers')+theme(legend.position='none')
ggplot(vis,aes(type,ESS))+geom_boxplot(aes(color=type))+labs(x='Sampler type',y='log(ESS)',title='Comparison of ESS for both types of samplers')+theme(legend.position='none')
